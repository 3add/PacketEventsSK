...
public class ExprFakeMetaGravity extends EntityMetaPropertyExpression<EntityMeta, Boolean> {
                        ...
                        command flyingItem <itemtype>:
                            trigger:
                                ...
                                spawn new fake dropped item at player for players:
                                    set fake item type of fake entity to {_i}
    ...
    protected Boolean @Nullable [] getMetaProp(Event event, EntityMeta meta) {
        return new Boolean[]{!meta.hasNoGravity()}; // inverted since the property is "gravity state" but the underlying meta uses "no gravity"
    }
    ...
    protected void changeMeta(Event event, EntityMeta meta, Object @Nullable [] delta, Changer.ChangeMode mode) {
        ...
        meta.setHasNoGravity(!newState); // inverted since the property is "gravity state" but the underlying meta uses "no gravity"
    }
